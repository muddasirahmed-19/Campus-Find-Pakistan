...
class _EmailSignupScreenState extends State<EmailSignupScreen> {
  ...
  Future<void> _signIn() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _isLoading = false; _errorMessage = _mapSignInError(e.code); });
    } catch (_) {
      if (!mounted) return;
    ...
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text('Your email is not verified yet.\n\n'
          'Please click the verification link sent to your Gmail inbox.',
          ...
          onPressed: () async {
            ...
            try {
              ...
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                content: Text('Verification email resent. Check your inbox and spam folder.')));
            } catch (_) {}
          },
  ...
  // Forgot password — checks if email is registered first
  void _showForgotPasswordDialog() {
    ...
    showDialog(context: context, builder: (ctx) {
      return StatefulBuilder(builder: (ctx, setS) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          Text('Enter your registered Gmail address to receive a password reset link.',
          ...
          ElevatedButton.icon(
            onPressed: sending ? null : () async {
              ...
              try {
                ...
                setS(() { sending = false; dialogError = 'Something went wrong. Try again.'; });
              }
            },
            icon: sending
...
class _CreateAccountScreenState extends State<CreateAccountScreen> {
  ...
  // Keep password in memory for verification check after dialog
    ...
    showDialog(
      ...
      builder: (ctx) => _VerificationDialog(
        ...
        onVerifiedAndLoggedIn: () {
          // Already signed in inside dialog
          Navigator.pop(ctx);
          Navigator.pushAndRemoveUntil(context,
            MaterialPageRoute(builder: (_) => const HomeScreen()), (_) => false);
        },
...
class _VerificationDialog extends StatefulWidget {
  ...
  const _VerificationDialog({
    ...
    required this.onVerifiedAndLoggedIn,
...
class _VerificationDialogState extends State<_VerificationDialog> {
  ...
  // When user closes dialog — check if verified, if yes auto login
  Future<void> _onClose() async {
    ...
    try {
      ...
      if (fresh != null && fresh.emailVerified) {
        // Verified — stay signed in and go home
        widget.onVerifiedAndLoggedIn();
        return;
      }
      // Not verified — sign out and just close
      await FirebaseAuth.instance.signOut();
    } catch (_) {
      await FirebaseAuth.instance.signOut();
    }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      actions: [
        ...
        // Close — auto checks verification and logs in if verified
        ElevatedButton(
